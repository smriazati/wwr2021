...
function custom_page_scripts() {
	...
	if( is_shop() ){
		// wp_enqueue_script( 'selectWooCDN', 'https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js', '', '', false );
	...
	wp_enqueue_script( 'wwr2021-sitewide', get_template_directory_uri() . '/js/site.js', array(), _S_VERSION, true );
...
// add image wrapper around images on archive & single shop pages 
function archive_products_image_wrapper_start() {
    echo '<div class="image-wrapper">';
}

function archive_products_image_wrapper_end() {
    echo '</div>';
}
add_action('woocommerce_before_shop_loop_item_title', 'archive_products_image_wrapper_start', 9);
add_action('woocommerce_before_shop_loop_item_title', 'archive_products_image_wrapper_end', 11);
